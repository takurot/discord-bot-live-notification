...
- ✅ P1-T12: 通知送信サービス実装 ⭐ NEW

**次のタスク:** ⏳ P1-T13: 配信終了時の通知更新機能
...
- **内容:**
  ...
  - Twitch情報（タイトル/カテゴリ/視聴者数/サムネイル）をEmbed化
  - ロールメンション・カスタムメッセージに対応
  - 送信メッセージIDをDB保存（配信終了時の更新フラグ）
  - `bot/index.ts` に通知サービスを統合
  - `jest.config.js` の `testMatch` と各コマンドテストを Discord.js v14仕様へ更新
- **依存:** P1-T11
- **並列可能:** ❌
...
- **実装結果:**
  - 通知サービス単体カバレッジ 94.28%
  - 実配信で `/notify add` → 配信開始通知まで確認済み
  - 今後の配信終了更新（P1-T13）用に messageId を保存済み
