...
**次のタスク:** ⏳ P1-T06: `/notify add` コマンド実装
...
### **Phase 1: MVP - Twitch対応・Freeプラン基本機能** 【進行中: 5/19タスク完了】

**進捗状況:** 
- ✅ 完了: 5タスク（P1-T01 〜 P1-T05）
- ⏳ 次: P1-T06 `/notify add` コマンド実装
- 📊 進捗率: 26% (5/19)
...
#### **⏳ P1-T06: `/notify add` コマンド実装** 【次のタスク】
...
- **内容:**
  ...
  - 5分間隔で全Subscription対象の配信ステータスをチェック
  - 配信開始検知時にイベント発行（`streamStarted`）
  - 配信終了検知時にイベント発行（`streamEnded`）
  - DB更新（`last_status` フィールド更新）
  - エラーハンドリング（API障害時のリトライ、ログ出力）
- **依存:** P1-T04, P1-T05
- **並列可能:** P1-T06以降と並行可能 ✅
- **テスト:** ユニットテスト（モックAPI）、統合テスト（実API）
- **PRサイズ:** 中（5〜8ファイル）
