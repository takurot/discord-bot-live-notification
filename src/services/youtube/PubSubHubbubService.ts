...
function isPubSubFeed(data: unknown): data is PubSubFeed {
  ...
  return entry === undefined || Array.isArray(entry);
}
...
export class PubSubHubbubService {
  ...
  async handleNotification(req: Request, res: Response): Promise<void> {
    try {
      ...
      const parsed: unknown = await parseStringPromise(xmlBody);
      if (!isPubSubFeed(parsed) || !parsed.feed?.entry?.length) {
        ...
        if (stream && stream.startedAt) {
          ...
          if (streamer) {
            ...
            if (!wasLive) {
              ...
              logger.info(
                `Streamer ${streamer.username} is already marked as Live. Skipping notification.`
              );
            }
          }
        } else {
